...
class FaceDetector
{
	...
	private function initClassifier($classifierFile)
	{
		...
		foreach($stagesNode  as $stageNode)
		{
			$stageThreshold = $stageNode->getElementsByTagName("stage_threshold")->item(0);
			$left_val = $stageNode->getElementsByTagName("left_val")->item(0);
			$right_val = $stageNode->getElementsByTagName("right_val")->item(0);
			$stage = new Stage(floatval($stageThreshold->nodeValue));
			$trees = $stageNode->getElementsByTagName("trees");
			foreach($trees  as $treeNode)
			{
				$feature = new Feature(floatval($stageThreshold->nodeValue), floatval($left_val->nodeValue), floatval($right_val->nodeValue), $this->classifierSize);
				$_1 =$treeNode->getElementsByTagName("_")->item(0);
				$f=$_1->getElementsByTagName("feature")->item(0);
				$rects =$f->getElementsByTagName("rects")->item(0);;
				$_ =$rects->getElementsByTagName("_");
				
				foreach($_ as $r)
				{
					$feature->add(Rect::fromString(strval($r->nodeValue)));
				}
